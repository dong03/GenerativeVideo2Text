...
class gvt_video_caps_dataset(Dataset):
...
class gvt_images_caps_dataset(Dataset):
    def __init__(self, ann_file, video_root, max_words=30, read_local_data=True, is_train=True, num_frm=4, transform=None,
        ...
        self.video2id = {}
        idx = 0
        ann = open(ann_file).readlines()
        self.ann = [each.strip() for each in ann]
    ...
    def __getitem__(self, index):
        try:
            ...
            video_path = ''
        return video, video_path.split('/')[-1]
        # import pdb; pdb.set_trace()
