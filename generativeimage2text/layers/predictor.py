        ...
        for step in range(max_length):
            ...
            if do_sample:
                ...
                topk_ids = torch.multinomial(
                    F.softmax(_scores, dim=-1), num_samples=1)
                # Compute next scores
                # (batch_size * num_beams, vocab_size)
                _scores = F.log_softmax(_scores, dim=1)

                _scores += topk_log_probs.view(-1).unsqueeze(1)
                # (batch_size * num_beams, 2)
                topk_scores = torch.gather(_scores, -1, topk_ids)
                # log_probs +=   # (batch_size * num_beams, 2)
                # Match shape of greedy beam search
                # (batch_size, 2 * num_beams)
                topk_ids = topk_ids.view(-1, beam_size)
                # (batch_size, 2 * num_beams)
                topk_scores = topk_scores.view(-1, beam_size)
            else:
                ...
                for i in range(batch_size):
                    for previous_token in set(input_ids[i].tolist()):
                        ...
                        if next_token_logits[i, previous_token] < 0:
                            next_token_logits[i,
                                              previous_token] *= repetition_penalty
                        else:
                            next_token_logits[i,
                                              previous_token] /= repetition_penalty
